...
class Eoptimization:
    ...
    def loadEdata(self):
        ...
        tdata=self.influxclient.query('SELECT mean("value") as temperature, time as time from "°" WHERE "entity_id"=\''+self.influxconfig['outside_temperature_sensor']+'\' and time <= now() and time >= now() - 365d GROUP BY time(1h)')['°C']
